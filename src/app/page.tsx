import { Calendar, AlertCircle, Shirt, ChevronRight } from "lucide-react";

export default function Home() {
  const schedule = [
    { time: "10:00 AM", title: "Focus Session", description: "Deep work: Design System", color: "bg-primary" },
    { time: "12:30 PM", title: "Meal Prep with Spinach", description: "Health: Use items expiring today", color: "bg-slate-200 dark:bg-slate-800" },
    { time: "4:00 PM", title: "Evening Walk", description: "Minimal living routine", color: "bg-slate-200 dark:bg-slate-800" },
  ];

  const alerts = [
    { icon: "🍃", name: "Spinach", status: "Today", color: "text-red-500", bgColor: "bg-red-50 dark:bg-red-900/10" },
    { icon: "🥛", name: "Fresh Milk", status: "2 days", color: "text-orange-500", bgColor: "bg-orange-50 dark:bg-orange-900/10" },
  ];
    ...
    <div className="px-6 pt-12 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="space-y-4 relative before:absolute before:inset-y-0 before:left-3 before:w-px before:bg-slate-200 dark:before:bg-slate-800">
          {schedule.map((item, idx) => (
            <div key={idx} className="flex gap-6 relative pl-8">
              <div className={`absolute left-0 top-1.5 w-6 h-6 rounded-full border-4 border-background-light dark:border-background-dark ${item.color}`} />
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-end">
          ...
          <button className="text-xs font-bold text-primary">View Pantry</button>
        </div>
        <div className="grid grid-cols-2 gap-4">
          {alerts.map((item, idx) => (
            <div key={idx} className="glass p-5 rounded-3xl flex flex-col items-center text-center space-y-3">
              <div className={`w-14 h-14 ${item.bgColor} rounded-full flex items-center justify-center text-2xl`}>
                {item.icon}
              </div>
              <div>
